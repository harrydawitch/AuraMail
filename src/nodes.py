...
class Nodes:
    ...
    def alert_email(self, state: State):
        search_tool = GmailSearch(api_resource= self.gmail.api_resource)
        search_results = search_tool.invoke("label:inbox")[0] # -> return a dictionary
        
        goto = "classifier"
        update = {
            "input_email": search_results
        }
    ...
    def classifier(self, state: State):
        ...
        # state["input_email"] = search_results 
    ...
    def interrupts_handler(self, state: State):
        ...
        elif request.get("type") is False:
            goto = END
            update = {
                "decision": "ignore"
            }
