...
class Workflow:
    ...
    def _create_workflow(self): 
        ...
        alert_email        = self.Node.alert_email
        ...
        self.graph.add_node("alert_email", alert_email)
        ...
        # Connecting nodes
        self.graph.add_edge("alert_email", "classifier")
        ...
        self.graph.add_conditional_edges(
            "interrupts_handler", lambda state: state["decision"],
            {"response": "writer", "ignore": END}
        )
        
        self.graph.add_edge("writer", "send_response")
        self.graph.add_conditional_edges(
            "send_response", lambda state: state["decision"],
            {"response": END, "rewrite": "writer"}
        )


        # Set entry point
        self.graph.set_entry_point("alert_email")
